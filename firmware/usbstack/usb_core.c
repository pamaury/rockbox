...
static void request_handler_device(struct usb_ctrlrequest* req)
{
    ...
    switch(req->bRequest) {
        ...
        case USB_REQ_CLEAR_FEATURE:
            logf("usb_core: CLEAR DEVICE FEATURE 0x%x 0x%x", req->wValue, req->wIndex);
            logf("+------+");
            logf("| oops |");
            logf("+------+");
            break;
...
static void request_handler_interface_standard(struct usb_ctrlrequest* req)
{
    switch (req->bRequest)
    {
        ...
        case USB_REQ_GET_INTERFACE:
            ...
            break;
        case USB_REQ_CLEAR_FEATURE:
            logf("usb_core: CLEAR INTERFACE FEATURE 0x%x 0x%x", req->wValue, req->wIndex);
            logf("+------+");
            logf("| oops |");
            logf("+------+");
            break;
        case USB_REQ_SET_FEATURE:
            logf("usb_core: SET INTERFACE FEATURE 0x%x 0x%x", req->wValue, req->wIndex);
            logf("+------+");
            logf("| oops |");
            logf("+------+");
            break;

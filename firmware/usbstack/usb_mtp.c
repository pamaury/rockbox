...
static int storage_id_to_volume(uint32_t stor_id)
{
    /* assume valid storage id */
    return (stor_id & 0x0000ffff) - 1;
}

static bool is_valid_storage_id(uint32_t stor_id)
{
...
static void probe_storages(void)
{
    int i;
    char buffer[32];
...
static inline const char *get_volume_identifier(uint32_t stor_id)
{
    (void) stor_id;
    return "";
}
...
static inline void probe_storages(void)
{
}

static inline int storage_id_to_volume(uint32_t stor_id)
{
    /* assume valid storage id */
    return 0;
}

static inline int volume_to_storage_id(int volume)
{
    /* assume valid volume */
    (void) volume;
    return 0x00010001;
...
static void get_partial_object(uint32_t object_handle,uint32_t offset,uint32_t max_size)
{
    const struct dircache_entry *entry = mtp_handle_to_dircache_entry(object_handle, false);
    static struct get_object_st st;
    static char buffer[MAX_PATH];
...
static void handle_command2(void)
{
    ...
    switch(cur_cmd.code)
    {
        ...
        case MTP_OP_GET_PARTIAL_OBJECT:
            ...
            want_session(SEND_DATA_PHASE)
            return get_partial_object(cur_cmd.param[0],cur_cmd.param[1],cur_cmd.param[2]);
